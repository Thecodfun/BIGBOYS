   ...
   const PREFIX = "!"
       ...
       client.user.setActivity(`Big Boy's Server! | Do !help`, {
           type: 'WATCHING'
       })
   });
   ...
   client.on("guildMemberAdd", (member) => {
       ...
       let role = client.guilds.get("698863825121837076").roles.get("699420767393153034")
           member.addRole(role)
   });
   ...
   client.on("message", async message => {
       ...
       if (command === "ping") {
           if (!message.member.hasPermission('ADMINISTRATOR'))
               return message.channel.send((":x: You can't ping the bot! :x:")).then(msg => msg.delete(2000))
       ...
       if (command === "pudelete") {
           if (message.member.hasPermission("MANAGE_MESSAGES" || "ADMINISTRATOR")) {
               var member = message.mentions.members.first()
                   ...
                   if (!member && !amount && !args[0])
                       return message.reply('Must specify a user to purge and a number of messages to purge!');
                   if (!member)
                       return message.reply('Must specify a user to purge!');
                   if (!amount)
                       return message.reply('Must specify an amount of messages to delete!');
                   if (args[1] > 99)
                       return message.reply('You **CANNOT** delete more than 99 messages!');
